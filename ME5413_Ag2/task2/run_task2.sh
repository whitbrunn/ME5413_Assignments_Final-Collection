...
tmux new-session -d -s rosbag_session "
  cd ~; 
...
gnome-terminal --tab -- bash -c "
  ...
  source ~/ctgrapher_ws/devel_isolated/setup.bash
  roslaunch cartographer_ros demo_jackal_bag.launch bag_filename:=/home/jy/data/task2.bag;
  exec bash
"
...
tmux send-keys -t rosbag_session C-c
sleep 5 
tmux kill-session -t rosbag_session
...
cp /home/jy/ctgrapher_ws/src/cartographer_ros/cartographer_ros/configuration_files/demo_jackal_bag.lua $LUA_PATH
evo_ape bag $BAG_PATH /ground_truth /tf:map.base_link --align --plot --save_plot ${EVO_PLOT_PATH} > score.txt
cp -f score.txt $SCORE_TXT_PATH
